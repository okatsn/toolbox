...
%         error('Under Construction...');
        ...
        [NoV,minId] = min(sizeinputXY); % NoV: the number of variables
    ...
    case {'matlab.io.MatFile','char'}
        ...
        % this makes WWidth the multiple of AvgWindowWidth in order to be correctly reshaped
...
if id1 > maxsize
    id1 = maxsize;
    % in each iters there're exactly WWidth points to be reshaped.
    nanArray = NaN(WWidth - id1,1); % to make [mtf.X;nanArray] total WWdith points.
    if total_iter ~= 1
        % if WWidth is larger than maxsize, then there must be only 1 loop
        % (i.e. total_iter == 1)
        error('You should not see this. Something must go wrong.');
    end
end
...
%                 catch
%                    disp('Set break point here to debug...') 
%                 end
            end
        else %maxdim =2
            for i = 1:NoV
    ...
    if minid~=1 % if minid == 2, next loop is the last loop
