...
if ~isequal(SaveInplace,0)
    ...
    totalTime_j = min([totalTime,floor(memoryLimit*dt)]);
...
%         FileExist = false;
        ...
        Y = [];
        StartPoint = [];
        varNames = {'D','r','dt','constantForce','FrictionType','StartPoint','X','Y'};
...
% '\d+\.?\d*': �Ʀr(�ܤ�1) �p���I(1��0��) �Ʀr(0�ܵL����) 
% 'e?[+-]?\d*\.?\d*': �r��e(1��0��) (�r�Ŷ�)[+��-](1��0��) �Ʀr(0�ܵL����) �p���I(1��0��) �Ʀr(0�ܵL����) 
% '(?=(pt|sec))': �᭱�n����pt��sec�~�|�ǰt��
...
%     assignin('base','smooth_parameter',smooth_parameter);
    switch smth_to
        case 'pt' % smooth to total m point
            m = smooth_parameter;
            tmp = totalTime/m/dt;
            steps=round(tmp); %Euler method�]n�B(�@N+1����ơA�Ĥ@������l����)
...
%             assignin('base','m',m); % DEBUG
        ...
        case 'sec' % smooth window (in seconds) = smooth_parameter
            ...
            steps=round(smooth_parameter/dt); %Euler method�]n�B(�@N+1����ơA�Ĥ@������l����)
            Nw=steps+1; % points in each window
            T_smth = NaN(m,1); Y_smth = NaN(m,1);
...
if isstruct(results.seed)
   ...
   O.seed=rng;%�x�s�H���ܼƪ��ؤl�A�H�K�ݭn�ɯ୫�{�C
end
...
% if isequal(SaveInplace,0)
for k=1:m
    dW=sigma.*randn(1,steps)+mm;%�����Ȭ��s%�@������n���H���ܼ�(1�Cn��)�CNw=steps+1;
%     dWm{k} = dW; % for debug use.
    ...
    traceT(1)=t;    traceY(1)=y;%initial condition
        for i=2:Nw
            ...
            switch FrictionType
                case {'dry','Coulomb'}
                    % Prox / soft-threshold step for Coulomb friction
                    z = y + Fext_x_dt + b*dW(i-1);
                    y = sign(z) * max(abs(z) - r*dt, 0);
                otherwise
                    % Original Euler–Maruyama for viscous or other types
