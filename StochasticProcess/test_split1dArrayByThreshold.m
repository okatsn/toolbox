%% test split1dArrayByThreshold and TsSegStat
% How to use:
...
function test_split1dArrayByThreshold(varargin)
nopass = 0;
if nargin > 0
    firstArg = varargin{1};
    ...
    maxY = max(abs(Y));
    thr_test1 = [-1e19 0];
    [segments_all,desired_id,undesired_id] = split1dArrayByThreshold(Y,0);
    [segments_all2,desired_id_2,undesired_id_2] = split1dArrayByThreshold(Y,[-1e19 0]); % they should be the same.
%     test 1:
    condition1 =  sprintf('thr = 0 and [%.2e %.2e]',thr_test1(1),thr_test1(2));
    fprintf('Test 1: testing split1dArrayByThreshold with %s.\n',condition1);
    ...
    if ~isequaln(desired_id,desired_id_2)
        warning("Test 1-2 failed ('desired' not identical for %s)",condition1);
        nopass = nopass+1;
    end
    
    if ~isequaln(undesired_id,undesired_id_2)
