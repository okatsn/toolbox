function [ax,hlines,fig] = figplotyyy_0(x1,y1,x2,y2,x3,y3,varargin)
...
warning('[figplotyyy0] This function should be deprecated.')
...
limx1=get(ax(1),'xlim');
limx3=[limx1(1)   limx1(1) + (offset*10*limx1(2)-limx1(1))];
...
axes(ax(2)) % put the layer of ax(2) to the very front. (Before it is ax(3) in the very front)
...
[ax(4)] = dupAx(ax(3),'Shift',ShiftYAxis);
ax(4).YLabel = ax(3).YLabel;
ax(3).YTickLabel = {};
ax(3).TickDir = 'out';
% ax3.TickLength = [0,0];
% ax3.XColor = 'none'
...
% A = {ax(2:3).YTickLabel};
% nelements = cellfun(@numel,A);
% [maxv,maxid] = max(nelements); % find which one have the more Y labels.
% [minv,minid] = max(nelements); % find which one have the more Y labels.
% maxid = maxid+1;
% minid = minid+1;
% % NoYlb = [NaN,5,6];
% % for i = 2:3
% %     NoLbi = NoYlb(i);
%     axlbs_new_d = ax(maxid).YTickLabel;   
% 
%         axlbs_new_d{1} = '';
%         
%         if rem(maxv,2)==0
%             % if remainder is zero, then do nothing
%         else
% %             try
%             axlbs_new_d{maxv/2+0.5} = '';        
% %             catch
% %                 a = 1+1
% %             end
%         end
%         axlbs_new_d{end} = '';
% %     end
%     ax(maxid).YTickLabel = axlbs_new_d;
%     ax(maxid).TickDir = 'in';
%     ax(minid).TickDir = 'out';
%     ax(maxid).TickLength = ax(maxid).TickLength*5;
%     ax(maxid).YMinorTick = 'off';
% % end
